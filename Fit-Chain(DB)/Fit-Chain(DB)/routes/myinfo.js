...
router.get('/', function (req, res, next) {
  if (req.session.profile_img_path) {
    ...
    conn.connect((err) => {
      ...
      const sql_info_user = `select * from member where name="${req.session.userName}"`;
      const sql_gallery_user = `select * from main_photo where name = "${req.session.userName}"`;

      conn.query(sql_gallery_user, (err, rs2, fields) => {
        if (err) {
      ...
      conn.query(sql_info_user, (err, rs, fields) => {
        if (err) {
          console.log(err.message);
          result.msg = "ERROR발생";
        } else {
...
router.post('/', function (req, res, next) {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    conn.connect((err) => {
      ...
      // 3. SQL문 생성
      const sql = `update member set profile_img_path = "${profile_img_path}" where name = "${req.session.userName}";`;
      console.log(sql);
      // 4. SQL문 전송
