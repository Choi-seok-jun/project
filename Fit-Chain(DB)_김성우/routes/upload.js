...
router.post('/', function (req, res, next) {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    console.log(" upload_file_type: "+form.openedFiles[0].type);
    var upload_file_type = form.openedFiles[0].type;
    if(upload_file_type.indexOf('image') != -1){

    if(err) {
      return res.redirect('/main#main_photo');
    }
    ...
    //-------------------------------------------------------
     ...
     console.log("업로드 파일 형식이 이미지가 아닙니다!!");
     res.redirect('/main#main_photo');
