...
router.get('/', function (req, res, next) {
  ...
  conn.connect((err) => {
    ...
    // 3. SQL문 생성
    const sql = `select * from main_photo where name = ?`;
    console.log(sql);

    // 4. SQL문 전송
    conn.query(sql,[req.session.userName], (err, results, fields) => {
      if (err) {
...
router.post('/', function (req, res, next) {
  ...
  conn.connect((err) => {
    ...
    // ----------------------------------------------------------
    conn.query(sql,[req.body.charge_coin,req.session.userId],(err, rs, fields) => {
      if (err) {
